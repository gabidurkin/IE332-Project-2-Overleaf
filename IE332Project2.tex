...
All experimental variants of your
...
\indent In our algorithm, we selected 5 algorithms that will perform adversarial attacks collectively to trick the binary image classifier. After intensive research, we selected the Genetic Algorithm (GA), Particle Swarm Optimization (PSO), Simulated Annealing (SA), Gradient Descent and Random Forest as our algorithms to attack the classifier. The Genetic Algorithm is a heuristic that is based on natural selection where the fittest individuals are selected to pass on characteristics to the next generation. We selected this algorithm because XYZ. Particle Swarm Optimization XXX. We selected this algorithm because XYZ. Simulated Annealing is XXX. We selected this algorithm because XYZ. Gradient Attack is XXX. We selected this algorithm because XYZ. Lastly, Random Forest is XXX. After being familiarized with it in Assignment 2, we decided to implemement it as our fifth algorithm. 
