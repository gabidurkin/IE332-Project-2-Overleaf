...
\noindent The code above implements a weighted majority classifier algorithm to combine the results of five different optimization techniques to generate an output image that is optimized for a given input image and a specified pixel budget. The first five lines of the code apply the five different optimization techniques (genetic algorithm, particle swarm optimization, simulated annealing, fast gradient sign method, and hill climbing) to the input image to generate optimized results for each technique. These optimized results are stored in separate variables. The code then assigns weights to each of the optimized results based on their performance. The weights are specified using the weight\_ga, weight\_pso, weight\_sa, weight\_gd, and weight\_rf variables. The weights are then combined into a vector called weights. The weighted\_results variable is then computed by taking a weighted sum of each of the optimized results using the weights in the weights vector. This is the core of the weighted majority classifier algorithm. The resulting weighted sum is a single image that represents the combination of the optimized results. The select\_pixels function is then used to select the pixels to modify based on the weighted\_results and the pixel\_budget. This generates the final output image that is optimized according to the specified budget and the user-specified weights for each optimization technique. In summary, the weighting in this code occurs by assigning weights to each of the optimized results from the different optimization techniques and combining these weights into a vector. This vector is then used to compute the weighted sum of the optimized results to generate the final output image. The result assigns weights to different algorithms based on the expected performance of some images; after the algorithms vote on which  pixels to change, the optimizer must choose from that set which pixels to actually use.
...
\noindent After this realization, the team improved this new algorithm by breaking down the finding of optimal pixels into smaller iterations. Our improved algorithm now finds very few optimal pixels, changes them on a gradient, then repeats this iteration until the 1\% pixel budget is reached. This allows pixels that have been altered once to be altered again if it will more efficiently confuse the classifier. Because the same pixel can be adjusted more than once, the algorithm can iterate more times than originally, while still remaining below the pixel budget. This update greatly improved our results by causing misclassifications more often and with greater inaccuracy.
...
\end{center}
\begin{center}
Table 2. Grass Image Accuracy Before and After Attacks
\end{center}
...
\end{center}
\begin{center}
Table 3. Dandelion Image Accuracy Before and After Attacks
\end{center}
...
The runtime of an algorithm in R that changes the pixels of an image to modify it would depend on various factors such as the size of the image, the complexity of the image processing techniques used, and the computational power of the system on which the algorithm is run. In general, image processing algorithms involve operations such as loading the image, manipulating pixel values, applying filters or transformations, and saving the modified image. These operations can be computationally intensive, especially for large images. To optimize the performance of the algorithm, techniques such as parallel processing, vectorization, and caching can be used. Our runtime is XXX and with this runtime we are able to process splitting the image processing task into smaller subtasks that can be executed concurrently on multiple processor cores. With this runtime XXX the vectorization involves performing operations on arrays of data instead of individual elements, which can significantly improve the processing speed. Caching can be used to store intermediate results to avoid recomputing them. In conclusion, our runtime XXX of the algorithm that changes the pixels of an image in R can vary depending on various factors, and optimizing the algorithm's performance can involve techniques such as parallel processing, vectorization, and caching.
